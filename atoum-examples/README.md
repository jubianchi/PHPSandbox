...
Rappelez-vous bien une chose : nous sommes dans le cadre de tests unitaires et il serait très dommage de coupler ces tests à l'environnement sur lequel ils sont exécutés, c'est donc là que les Adapters vont nous aider.
...
Notre code y gagne en testabilité et donc en fiabilité si les tests adéquat sont mis en place. La contrepartie, c'est que nous sommes désormais dépendant de l'Adapter et que cela peut nous jouer des tours dans certains cas (les performances peuvent par exemples être impactées).
...
Comme vous pouvez le voir, la dépendance vers atoum a disparue de notre code de production ! Nous sommes maintenant dépendant de notre ```Adapterinterface``` et de l'implémentation par défaut que nous avons ajoutée.